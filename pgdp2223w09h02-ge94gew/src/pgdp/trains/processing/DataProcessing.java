...
public class DataProcessing {
    ...
    public static Stream<TrainConnection> cleanDataset(Stream<TrainConnection> connections) {
        ...
        return connections.distinct().sorted(Comparator.comparing(o -> o.stops().get(0).scheduled()));
    }

    public static TrainConnection worstDelayedTrain(Stream<TrainConnection> connections) {
        if (connections == null) return null;
        return connections.max(new Comparator<TrainConnection>() {
            @Override
            public int compare(TrainConnection o1, TrainConnection o2) {
                return o1.stops().stream().mapToInt(TrainStop::getDelay).sum() - o2.stops().stream().mapToInt(TrainStop::getDelay).sum();
            }
        }).orElse(null);
    }
