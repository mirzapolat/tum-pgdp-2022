...
public class TrialOfTheSword {
	...
	public static class FlatArray<T> {
		...
		public int computeIndex(int... idxs) {
			...
			return computeIndexRec(idxs, dims);
		}

		private int computeIndexRec(int[] idxs, int[] curDims) {
			if (idxs.length == 1) return idxs[0];

			int myProd = idxs[0];
			for (int i = 1; i < curDims.length; i++) {
				myProd *= curDims[i];
			}

			int[] newIdxs = new int[idxs.length - 1];
			for (int i = 1; i < idxs.length; i++) newIdxs[i-1] = idxs[i];
			int[] newDims = new int[curDims.length - 1];
			for (int i = 1; i < curDims.length; i++) newDims[i-1] = curDims[i];

			return myProd + computeIndexRec(newIdxs, newDims);
		}
