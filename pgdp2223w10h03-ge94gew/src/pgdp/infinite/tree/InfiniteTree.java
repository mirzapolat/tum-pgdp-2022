...
public class InfiniteTree<T> {
    ...
    public T find(T from, int maxDepth, Optimizable<T> optimizable) {
        if (optimizable.process(from)) return from;
        if (maxDepth == 0) return optimizable.getOptimum();

        InfiniteNode<T> root = this.withRoot(from);
        while (!root.isFullyCalculated()) {
            T cal = find(root.calculateNextChild().getValue(), maxDepth -1, optimizable);
            if (optimizable.process(cal)) return optimizable.getOptimum();
        }
